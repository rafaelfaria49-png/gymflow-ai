...
- **`goalAdjustments` validado nos dois sentidos, sem clamp silencioso:** o envelope `[-maxAbsoluteDeficitKcal, +400]` é verificado para **todos** os objetivos (um config é válido como um todo, independentemente de qual objetivo venha a consumi-lo); déficit fora do envelope agora é rejeitado em vez de clampeado. Os clamps em `computeTargetCalories` viraram redundantes por contrato e permanecem apenas como defesa para chamadas diretas da etapa pura.
- **Pisos calóricos deixaram de ser overrides públicos:** como o único valor justificável por documento canônico é o próprio valor canônico, o parâmetro não tinha liberdade semântica real. `femaleCaloricFloorKcal`, `maleCaloricFloorKcal` e `unspecifiedCaloricFloorKcal` saíram de `EngineCalculationConfig` e viraram invariantes em `ENGINE_HARD_SAFETY_LIMITS`, **continuando no `inputSnapshotHash`** (via `ResolvedCaloricFloors`): proveniência cobre todo input efetivo, configurável ou não.
- **Chave desconhecida em `EngineConfig` falha fechada:** novo `validateEngineConfigKeys` + código `UNKNOWN_CONFIG_KEY`. Um override removido do contrato (os pisos, `minFatCaloriePercentage`, `ketogenicFatCaloriePercentage`, `nonKetoCarbsFloorGrams`) passa a ser reportado como inexistente em vez de silenciosamente ignorado — o chamador não fica acreditando que o parâmetro está em vigor.
- **Regra relativa de piso `unspecified` removida:** com pisos constantes, `unspecified >= male -> MALE_DEFAULT_FORBIDDEN` era inalcançável e sustentava-se só por analogia (D-NUT-02 fala de fallback de fórmula/alvo, não de comparação entre pisos configuráveis). A garantia de D-NUT-02 passa a ser **estrutural** (1200 != 1500, verificado sobre os invariantes) e o código `MALE_DEFAULT_FORBIDDEN` segue em uso para `bmrUnspecifiedOffset`, que é literalmente a fórmula.
- **Lipídios ganham a faixa dura canônica:** `min/max/defaultFatGramsPerKg` passam a ser validados em `[0.7, 1.0] g/kg/dia`, faixa já escrita no Masterplan 8.2 — nenhum número novo.
- **Documentação canônica alinhada ao código:** Masterplan 7.1 passa a declarar `computedAt: string | null` + `computedAtSource`; Masterplan 7.2 documenta o teto de superávit, o teto de custo de treino, o piso de `unspecified` e a não-configurabilidade dos pisos; Masterplan 8.2 deixa de prometer "piso de segurança de carboidratos" e descreve a preferência best-effort real, com `NON_KETO_CARBS_PREFERENCE_UNMET`.

## NUT-003 — Corretivo de Safety e Proveniência (2026-09-11)

- **Invariantes absolutos deixam de ser configuráveis:** `ENGINE_HARD_SAFETY_LIMITS` separa parâmetros ajustáveis de travas duras (déficit <= 750 kcal, piso BMR >= 0.90, pisos 1200/1500/1200 kcal, proteína <= 2.2 g/kg, PAL 1.2–1.75, hidratação <= 4500 ml) e `EngineConfig` que tente ultrapassá-las falha fechada com `NutritionEngineConfigError`, sem clamp silencioso; a única exceção clampeada segue sendo o ajuste calórico por objetivo, cujo teto (`maxAbsoluteDeficitKcal`) passou a ser ele próprio limitado a 750 kcal.
- **Validação numérica antes do cálculo:** `validateProfileInputs`/`validateEngineConfig` rejeitam NaN, Infinity, -Infinity, negativos e zero semanticamente inválidos, config aninhado inválido e enumerações fora do contrato (eliminando os defaults ocultos de PAL sedentário e proteína 1.8 g/kg); `validateDailyTargetsOutput` é a barreira final, de modo que nenhum `DailyTargets` sai com número não finito, `targetWaterMl` negativo ou proteína acima de 2.2 g/kg.
- **Proveniência completa por construção:** `inputSnapshotHash` passou a cobrir a totalidade de `ResolvedCalculationConfig` (incluindo `trainingKcalPerMinute`, limites de proteína e `minHydrationMlPerDay`, antes ausentes), garantida por tipo e não por lista curada; o serializador canônico rejeita NaN/Infinity com `NutritionEngineSerializationError` em vez de deixá-los virar `null`.
- **`computedAt` honesto e identidade de evento separada:** o motor não deriva mais data de `profile.updatedAt` nem de literal hardcoded — sem contexto explícito o contrato emite `computedAt: null` com `computedAtSource: 'absent'` (divergência consciente do Masterplan 7.1, que previa `string`); `inputSnapshotHash` é a identidade determinística do snapshot e `DailyTargets.id` passa a derivar de snapshot + `computedAt` + `computedReason`, evitando colisão de eventos no ledger futuro.
- **Macros reconciliam ou declaram inviabilidade:** `macroReconciliation` expõe `macroCalories`, `deltaKcal`, tolerância de arredondamento do macro residual (2 kcal para carboidrato, 4.5 kcal para lipídio) e constraints não atendidas; o teto de 2.2 g/kg é reaplicado após o arredondamento inteiro e nenhuma regra clínica nova foi criada para escolher qual constraint sacrificar.
- **Tolerância de `unspecified` é guidance tipada:** `estimationTolerance` declara +/- 15% (Masterplan 5.2) com as bordas calóricas informativas, sem ajustar o alvo e sem qualquer fallback masculino.
- **Campos mortos removidos e semântica corrigida:** `minFatCaloriePercentage` e `ketogenicFatCaloriePercentage` saíram do contrato por não participarem do algoritmo aprovado, e `nonKetoCarbsFloorGrams` virou `nonKetoCarbsPreferenceGrams` — é preferência best-effort (cede ao lipídio essencial) e sinaliza `NON_KETO_CARBS_PREFERENCE_UNMET` quando não atingida, sem prometer piso clínico garantido.

## NUT-003 — Deterministic Engine + Targets (2026-09-09)

- **Pureza e proveniência determinística do NutritionEngine:** `calculateDailyTargets` aplica BMR Mifflin-St Jeor (com offset provisório -78 e piso 1200 kcal para `unspecified`), TDEE com PAL e treino, travas de segurança (déficit <= 750 kcal, BMR * 0.9 e pisos por sexo), proteína estritamente 1.6–2.2 g/kg, hidratação base 35 ml/kg (teto 4500 ml) e provenance SHA-256 canônica síncrona sem dependência externa, falhando tipado via `NutritionEngineGateError` em bloqueios ético-clínicos (registro histórico: as garantias efetivamente asseguradas pelo código são as do corretivo de 2026-09-11 acima — a faixa 1.6–2.2 g/kg é o padrão canônico, e apenas o teto de 2.2 g/kg é invariante duro).
...
- **Prova efêmera no Context:** o `LogicalStorageRestoreTargetV2` vive só em
  ref privada. Preview público tem apenas contagens agregadas. O commit
  re-resolve e recusa se a prova divergiu desde o preview.
- **Autosave e reload iguais ao E4B:** bloqueio antes do primeiro write;
  pendingSave cancelado; flush de ciclo de vida suspenso; sucesso settled
  mantém bloqueio e dispara exatamente um reload.
...
## GOAL-069 - flake de fronteira UTC/local no teste de storage (2026-09-12)

- D69-001: causa classificada como TEST_ONLY_ASSERTION_BUG — a producao ja derivava `todayIso` de `getCivilDateString(new Date(endedAt))` (data civil local); apenas a assercao de `GymFlowContext.storage.test.tsx` comparava com `new Date().toISOString().split('T')[0]` (projecao UTC). Nenhuma linha de producao foi alterada; a semantica de data civil local do dispositivo foi preservada.
- D69-002: determinismo obtido congelando **somente** `Date` (`vi.useFakeTimers({ toFake: ['Date'] })`) em vez de forcar TZ do processo — `setTimeout`/microtasks seguem reais porque a hidratacao hibrida e o `fake-indexeddb` dependem de temporizadores verdadeiros para assentar; nenhuma dependencia nova foi adicionada.
- D69-003: instantes congelados construidos com o construtor LOCAL (`new Date(2026, 8, 11, 23, 30)` e `new Date(2026, 8, 12, 0, 30)`), de modo que a data civil esperada e um literal valido em qualquer fuso; o par cobre os dois sinais de offset (offset negativo falha no caso 23:30, offset positivo falha no caso 00:30), provando ausencia de semantica UTC em todo fuso diferente de UTC sem condicional de horario.
- D69-004: os instantes absolutos exigidos pelo GOAL (`2026-09-12T00:01:00Z` -> `2026-09-11` e `2026-09-12T03:01:00Z` -> `2026-09-12` em `America/Sao_Paulo`) foram provados na camada de derivacao+gravacao (`storage-completion-receipt.test.ts`, com `timeZone` explicito), onde o fuso pode ser fixado sem depender do TZ do processo; `nutrition-civil-date.test.ts` foi preservado sem alteracao.


## GOAL-071 - residuais de data civil em UTC na producao do GymFlowProvider (2026-09-12)

- D71-001: os tres call sites (`addWeightLog`, `addMeasurementLog`, `unlockedAt` de `unlockAchievement`) passaram a usar o helper canonico ja existente `getCivilDateString()` sem argumento — nenhum helper novo foi criado e nenhum fuso foi hardcodado. A semantica permanece "data civil local do dispositivo/usuario", igual ao caminho puro `deriveWorkoutCompletion`, que ja gravava `unlockedAt` a partir de `input.todayIso`.
- D71-002: apenas campos date-only foram trocados. As tres ocorrencias restantes de `toISOString()` no arquivo (`recordedAt` em 2 pontos de override de progressao e `savedAt` do envelope de storage) representam instante real e foram classificadas INSTANT_OR_UTC_INTENTIONAL — nenhuma substituicao global foi aplicada.
- D71-003: a regressao ficou em `GymFlowContext.storage.test.tsx` (arquivo existente) e nao em arquivo novo, porque so ali existe o provider real com persistencia hibrida, `persistedCore()` e o helper `freezeClockAt` do GOAL-069; assim a prova cobre memoria, core persistido e reidratacao, e nao apenas o helper isolado.
...
- D117-007: achados da revisao independente (P1 x5, P2 x2) corrigidos no mesmo GOAL: AAB exige toda entrada assinada + signer unico; fluxo Play sem --skip-web e bundle web do AAB identico a out/ arquivo a arquivo (gate duro); arvore git limpa antes/depois do cap sync e na auditoria; caminhos canonicos (symlink/junction) antes da checagem de repo; backend nao embutido = FAIL salvo aceite humano explicito (--accept-backend-unavailable, registrado no manifest e no gate); senhas retiradas do env do processo; scan de valores literais de senha em todo arquivo versionado.
- D117-008: rodadas 2-4 da revisao independente fechadas no mesmo GOAL (P0 N6: .env* ignorado podia embutir backend arbitrario apos a trava do build:mobile). build:mobile valida o valor EFETIVO calculado com @next/env (adicionado como devDependency exata 16.2.6, uso recomendado pela doc do Next) e o fixa no next build; auditoria le as origens do resolvedor compilado (estrangeira = FAIL duro, waiver so com indisponibilidade comprovada); --allow-dirty amarrado ao certificado real de chave descartavel. N1 (backend nativo sem CORS) segue aberto por desenho: exige GOAL de backend + deploy autorizado.

## GOAL-118 - GYMFLOW-MOBILE-AI-BACKEND-PRODUCTION-BRIDGE-118

- D118-001: CORS do Assistente IA vive so no gateway da Nutricao (src/lib/nutrition/ai-assistant-route.ts; route.ts so delega POST/OPTIONS): allowlist exata https://localhost (Android, androidScheme https) e capacitor://localhost (iOS); origem ecoada, nunca *, sem Allow-Credentials; preflight so POST + Content-Type (Max-Age 600); CORS em toda resposta de origem nativa; Vary em toda resposta; next.config sem headers.
- D118-002: same-origin web = host do Origin igual ao header Host (nao forjavel por pagina) e esquema igual ao x-forwarded-proto quando presente; request.url nao e usado (no next start/dev ele pode nao refletir o Host). Chamadas sem Origin (curl/servidor/smoke) seguem o contrato de sempre, sem CORS.
- D118-003: origem nao autorizada e fail-closed tambem no servidor: POST -> 403 INVALID_REQUEST antes de ler o corpo (provedor nunca chamado; evita o gateway como proxy pago via POST simples text/plain de outra pagina); OPTIONS -> 403. Codigo INVALID_REQUEST reaproveitado (sem novo codigo de falha no contrato do client).
- D118-004: build:mobile com modo explicito --ai-backend production|none (padrao production) a partir da constante versionada PRODUCTION_BACKEND_ORIGIN; .env ignorado so pode repetir a Production (ou estar vazio no modo none); modo none remove a variavel do ambiente do next build. android:play:release passa o modo aprovado (--accept-backend-unavailable -> none; --expect-backend-production -> production) e confere com a mesma regra (planMobileBackend). Resolvedor do client inalterado (forma compilada reconhecida pela auditoria do GOAL-117).
- D118-005: deploy Production feito de worktree limpo do commit (so arquivos versionados), porque o diretorio de trabalho tem keystore interna, release-signing.properties e .env.local que nao podem subir para a Vercel como fonte.
- D118-006: smoke nativo no emulador Pixel_6_API_35 com o APK release auditado (S22 nao conectado ao ADB na execucao). Metas AUTOMATED obtidas por restore pelo fluxo do proprio app de um backup v2 gerado com as funcoes do projeto (nao ha UI de cadastro de perfil nutricional); estado QA do emulador substituido (predecessor mantido pelo app).
- D118-007: Privacy Manifest iOS e respostas do Data Safety NAO alterados; documentada proposta factual (coleta opcional de contexto nutricional quando o Assistente IA e usado) para decisao humana/juridica (D-NUT-09 PENDING).
- D118-008: rodada 1 da revisao independente (Codex GPT-5.6-Sol: P0=1 P1=2 P2=4 P3=1) tratada no GOAL: F2 exceção de QA do build removida (production = exatamente a Production); F3 variavel vazia/espacos normalizada antes da 1a leitura do @next/env e ambiente do next build montado do mesmo ambiente (resolveMobileBuild, teste de integracao em processo filho); F4 esquema same-origin cai para o protocolo da URL da requisicao sem x-forwarded-proto; F5 provedor so configurado com base URL HTTPS; F6 teto em bytes UTF-8 + 413 por Content-Length antes de ler o corpo; F7 excecao do explain_target_change documentada; F8 testes adicionados. F1 (gateway publico sem auth/quota) classificado como risco pre-existente do GOAL-110 nao ampliado pelo GOAL-118 (reduzido): correcao real exige backend (fora das regras permanentes); mitigacao humana (limite de credito OpenRouter + rate limit Vercel Firewall) registrada em PENDENCIAS.
- D118-009: rodada 2 da revisao independente: P0=0, P1=0 (F1 aceito como risco pre-existente P1 de backlog, nao atribuivel ao GOAL-118; F2-F8 FIXED). P2 novos corrigidos no GOAL: N1 leitura do corpo da requisicao com teto em bytes e stream cancelado sem Content-Length; N2 teto da resposta do provedor em bytes UTF-8 com leitura limitada (bounded-text.ts compartilhado); N3 fetch do provedor com redirect 'error'. N4 (docs) atualizado com o deploy e artefatos finais.
- D118-010: rodada 3 da revisao independente: P0=0, P1=0, P2=1 (M1). No modo none do build:mobile, a variavel DECLARADA vazia num .env* passa a ser recusada antes do build (effectiveBackendOrigin informa declared; planMobileBackend recusa), em vez de o Next embutir "" e a auditoria reprovar depois. Modo production inalterado (fixa a constante). Mudanca so de ferramental: gateway e bundle do modo production nao mudam, sem novo deploy.
