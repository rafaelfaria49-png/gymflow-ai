...
- **AI_RUNTIME_PROVIDER = UNCONFIGURED — BLOCKED_EXTERNAL_CREDENTIAL.** *Blocker externo legitimo (nao P0 de codigo), re-auditado no GOAL-109 sobre a master 649772f.* Fontes seguras verificadas (somente nomes, nenhum valor exposto): envs Vercel do projeto `gymflow` = apenas BLOB_READ_WRITE_TOKEN; `.env.local` = apenas VERCEL_OIDC_TOKEN; shell sem provider; C:\Projetos\secrets sem GYMFLOW_AI_*; projeto Vercel sem nenhum deployment. Variaveis faltantes exatas: GYMFLOW_AI_ENABLED, GYMFLOW_AI_BASE_URL, GYMFLOW_AI_API_KEY, GYMFLOW_AI_MODEL (+ NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL para o Capacitor). *Proximo passo (humano):* criar/fornecer a API key de um provider OpenAI-compatible em fonte segura e configurar as 4 envs server-only no Vercel (Production/Preview); so entao reexecutar deploy + smoke real dos 5 casos + negativos + mobile. NUT007_RUNTIME_STATUS=BLOCKED_EXTERNAL_CREDENTIAL; READY_FOR_NUT008=NO. Nao inventar chave, nao commitar segredo, nao iniciar NUT-008 nem OFF/barcode.
