...
- **Escolha manual de gerações** não existe: o predecessor após reset é
  o mundo anterior comprovado, sem seletor, histórico de resets ou
  delete de geração.
...
- ~~**Reset híbrido**~~ Fundação interna no E6A; UI entregue no E6B.
  Retenção executável continua bloqueada.
...
- ~~**AI_RUNTIME_PROVIDER = UNCONFIGURED.**~~ *Fechada no GOAL-110 (runtime OpenRouter VALIDATED em producao).*
...
## GOAL-109 - NUT-007 ativacao do runtime real (2026-09-16)

- ~~**AI_RUNTIME_PROVIDER = UNCONFIGURED — BLOCKED_EXTERNAL_CREDENTIAL.**~~ *Fechada no GOAL-110.* Runtime OpenRouter homologado em producao (gymflow / https://gymflow-beige-gamma.vercel.app, DEPLOY_SHA 3f1ed1f): 5 casos 200 grounded, negativos fail-closed, SECRET_EXPOSURE=NO, mobile via origem publica GymFlow. NUT007_RUNTIME_STATUS=VALIDATED; READY_FOR_NUT008=YES. Nao iniciar NUT-008 neste GOAL.
...
## GOAL-117 - Google Play Internal Testing onboarding (2026-09-24)

- ~~**BACKEND_PRODUCTION no Android nativo = NOT_CONFIGURED + CORS ausente.**~~ *Fechada no GOAL-118 (CORS nativo no gateway + deploy Production + origem Production embutida no build:mobile + smoke da IA pelo app; ver docs/mobile/ANDROID_AI_BACKEND_BRIDGE_118.md). Registro original, P1 do GOAL-117:* (1) build:mobile nunca recebeu NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL (nem .env.local, nem CI, nem script): o bundle le a variavel em runtime e a IA nativa resolve para `unavailable` (honesto, sem IA fake). (2) Gateway Production sem CORS: OPTIONS /api/nutrition/assistant com Origin https://localhost -> 204 sem Access-Control-Allow-Origin; POST idem. (3) O AI_BACKEND_SMOKE=PASS do GOAL-116 foi curl do shell do aparelho; o app nunca chamou o backend em nativo. *Proximo passo (GOAL de backend autorizado):* CORS allowlist (https://localhost Android, capacitor://localhost iOS) na rota do assistente -> deploy Production autorizado -> release com NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL=https://gymflow-beige-gamma.vercel.app (build:mobile ja so aceita essa origem) -> novo versionCode -> smoke da IA no aparelho -> atualizar Data Safety (dados de nutricao passam a sair do aparelho).
- **Upload key definitiva / backup / estado do Play Console.** *Aberto - gate humano, nao bug.* HUMAN_KEY_BACKUP_DECISION_REQUIRED: diretorio seguro, backup offline, cofre da senha e respostas P1-P6 de docs/mobile/ANDROID_PLAY_INTERNAL_ONBOARDING_117.md §2.
- **Revisao independente do ferramental (Codex / GPT-5.6-Sol, 6 rodadas + confirmacao).** Todos os achados P0/P1/P2 de ferramental corrigidos no proprio GOAL (ver ANDROID_PLAY_INTERNAL_ONBOARDING_117.md §10). Unico P1 remanescente = backend nativo acima (aberto por desenho, com waiver humano explicito amarrado ao artefato). *Proximo passo:* repetir a revisao sobre o AAB final assinado pela upload key real antes do gate do Play.

## GOAL-118 - Assistente IA nativo -> backend Production (2026-09-25)

- **1a chamada da IA apos troca de rede expirou no Wi-Fi do emulador.** *Aberto - P2 (verificacao em aparelho real).* No Wi-Fi virtual do Pixel_6_API_35, 4 de 5 primeiras chamadas apos troca da rede padrao expiraram no timeout do client (15 s, falha honesta, sem proposta fake); o AP virtual tinha beacon loss a cada ~8 s e desassociacoes espontaneas. Na rede celular do emulador passou 3/3; com rede estavel "Tentar novamente" funcionou. *Proximo passo:* repetir no Galaxy S22 (Wi-Fi real, troca Wi-Fi<->dados e offline->online). Se reproduzir: avaliar `android.permission.ACCESS_NETWORK_STATE` (o WebView so observa troca de rede com ela; hoje o manifest so tem INTERNET) e o orcamento de 15 s do client (DNS + TLS + preflight + ate 12 s do provedor).
- **Sem UI de cadastro do perfil nutricional.** *Aberto - P2 (produto).* O Provider so le `nutritionProfile` do estado persistido; no app, metas AUTOMATED (e portanto os casos permitidos do Assistente IA) so sao alcancaveis restaurando um backup com perfil. Usuario novo fica em PROFILE_ABSENT/MANUAL_ONLY. *Proximo passo:* GOAL de UI de perfil nutricional com os gates clinicos do NUT-002.
- **Copy do bloqueio por PROFILE_ABSENT.** *Aberto - P3.* O modal da IA mostra "Seu perfil exige acompanhamento profissional para metas automaticas" tambem quando o motivo e perfil ausente (nao ha perfil a avaliar). Honesto quanto a IA nao ter sido chamada, mas o motivo exibido e impreciso.
- **Dialogo de verificacao/importacao de backup fora da viewport no nativo.** *Aberto - P3 (UI).* No Admin (Android), o overlay `fixed` do StorageBackupVerifier fica contido no card "Dados locais" e o dialogo aparece abaixo da dobra (foi preciso rolar para ver "Importar este backup").
- **Privacy Manifest iOS e Data Safety do Play desatualizados em relacao ao fluxo nativo da IA.** *Aberto - gate humano/juridico (D-NUT-09 PENDING), nao bug.* `NSPrivacyCollectedDataTypes = []` e a resposta "nao coleta" do Data Safety assumiam app 100% offline; com o GOAL-118 o contexto nutricional sai do aparelho quando o usuario usa o Assistente IA. Proposta factual em MOBILE_CROSS_PLATFORM_005.md §9 e MOBILE_CROSS_PLATFORM_006.md §3.3. *Proximo passo:* decidir declaracoes antes de qualquer Internal Testing com usuarios alem do time e antes de submissao iOS; auditar retencao OpenRouter/provedor.
- **Smoke da IA no Galaxy S22.** *Aberto - P3.* O S22 nao estava conectado ao ADB; o smoke nativo foi no emulador com o mesmo APK auditado. Repetir no S22 junto do smoke do GOAL-117 §14.
- **Gateway da IA publico, sem autenticacao/quota (achado F1 da revisao independente do GOAL-118).** *Aberto - P1 operacional pre-existente (desde o GOAL-110); nao introduzido nem ampliado pelo GOAL-118.* Chamadas sem Origin (curl/bots) e qualquer WebView Capacitor (`https://localhost`) alcancam o gateway, que chama o provedor pago. O GOAL-118 reduziu a superficie (origem estrangeira de navegador -> 403 antes de ler o corpo); o uso e restrito (5 casos tipados, prompt do servidor, corpo <= 16 KiB, max_tokens 1200, saida filtrada pelo catalogo). *Mitigacao imediata (humano, fora do repo):* limite de credito na chave OpenRouter de GYMFLOW_AI_API_KEY + regra de rate limit do Vercel Firewall em /api/nutrition/assistant. *Correcao real (GOAL de backend autorizado):* autenticacao de usuario ou Play Integrity/App Attest + quota com estado; nunca segredo estatico no app.
