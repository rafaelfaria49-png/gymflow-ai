...
- **Fase:** 7 · **Prio:** P3 · **Depende de:** Gate G4 (D16, D17 encerradas).
- **Escopo e Natureza:** Exclusivamente documentação/ADRs e preparação neutra de domínio nos tipos, com zero código de backend, rede, auth, Supabase, billing ou chat.
- **Arquivos Criados em `docs/personal/`:**
  - `docs/personal/README.md`: Índice executivo, escopo da Fase 7 e integridade dos gates.
  - `docs/personal/SYNC_READINESS_AUDIT.md`: Auditoria objetiva de prontidão para sincronização com base no checklist de `GYMFLOW_SAAS_ARCHITECTURE.md` §1 (estabilidade de IDs, timestamps, snapshots vs referências, storage versionado e lacunas para a Fase 8).
  - `docs/personal/ADR-001-PAPEIS-E-PERMISSOES.md`: Matriz RBAC de 5 papéis e governança LGPD (Art. 11 — proteção estrita de dados sensíveis de saúde do aluno contra acesso não consentido de academias).
  - `docs/personal/ADR-002-FLUXOS-ESSENCIAIS.md`: Fluxo ponta a ponta do Personal (convites, builder para aluno, cockpit de aderência) e do Aluno no app unificado GymFlow AI (Single-App Architecture).
  - `docs/personal/ADR-003-ENTIDADES-E-ISOLAMENTO-RLS.md`: Especificação relacional das 7 entidades futuras do Supabase e isolamento multi-tenant via Row Level Security (RLS) com chave `org_id`.
  - `docs/personal/ADR-004-MODELO-COMERCIAL-E-PRICING.md`: Análise comparativa profunda dos Modelos A, B e C, fundamentação fiscal da Decisão D16 (Modelo A com faixas do Modelo C) e propostas de pricing para validação comercial.
  - `docs/personal/ADR-005-ACADEMIA-STUDIO-E-RISCOS.md`: Demarcação do escopo futuro Studio/Academia e matriz de mitigação dos 4 riscos críticos (R1 responsabilidade profissional CREF, R2 dados sensíveis LGPD, R3 foco de produto e R4 sobrecarga de suporte em tempo real).
  - `docs/personal/FASE_8_ROADMAP_GOALS.md`: Decomposição sequencial dos GOALs da Fase 8 (GOAL-36 ao GOAL-43) espelhando `GYMFLOW_SAAS_ARCHITECTURE.md` §6.
- **Único Ajuste em Código de Produto:**
  - Inclusão da propriedade neutra e opcional `createdBy?: 'user' | 'coach' | 'system'` na interface `WorkoutProgram` em `src/types/index.ts`.
  - Nenhuma outra alteração em código de produto. Nenhum campo antecipado no produto.
- **Testes e Serialização:**
  - Criado `src/lib/personal-sync-readiness.test.ts` com 4 testes cobrindo roundtrip JSON de `createdBy`, retrocompatibilidade com programas legados e persistência em envelopes de storage.
  - `storage.test.ts` e `storage-boot-recovery.test.ts` verificados.
- **Validação:**
  - `npx vitest run src/lib/personal-sync-readiness.test.ts` (4/4 aprovados).
  - `npx vitest run src/lib/storage.test.ts` (17/17 aprovados).
  - `npx tsc --noEmit` (0 erros).
  - `npm run build` (build web estático concluído com sucesso).
...
## GOAL-059 - Ingestao do video aprovado da Puxada Alta (2026-09-11)

- **Contexto:** ingestao do segundo video tecnico aprovado (`back_puxada_pulley`), com aprovacao humana explicita do operador neste handoff — resolve o gate `APPROVAL_NOT_PROVEN` dos GOALs 052/053. Base `0a03f85748a0503ff9df9c09622e31c7feb58a2f`.
- **Antes:** `back_puxada_pulley` em `draft` (placeholder `assets.gymflow.ai`); nenhum video da puxada no Blob.
- **Depois:** fonte `Puxada alta aberta.mp4` validada (720x1264, 145f @24fps, h264/yuv420p, AAC, 6.042s, sha256 `7101a20b…171da`, repCount 2 confirmado por inspecao de frames — puxada vertical aberta em polia alta, sem ambiguidade); normalizada deterministicamente (scale Lanczos 1094x1920 + crop 1080x1920, 24fps, h264, sem audio, +faststart, duracao preservada) e publicada na store existente `gymflow-media` como `back_puxada_pulley_v1.mp4` (4.148.838 B, sha256 `3a995473…0bbdf`, SHA remoto = local; HTTP 200, `video/mp4`, Range 206, CORS `*`); manifest version 3 → 4 (schema 1.1.0, sincronizados) com `back_puxada_pulley` em `approved` v1 (`approvedBy rafaelfaria49-png`, precisao `unknown`, sem `approvedAt`, evidencia `docs/GYMFLOW_VIDEO_INGEST_059.md#operator-approval`, sem model/run, sem claim comercial).
...
## GOAL-089 - NUT-004B exclusao cross-tab do admin via Web Locks (duracao alem do TTL) (2026-09-13)

- **Contexto:** elimina o P1 final do GOAL-088 sem depender da duracao do TTL, no mesmo branch do PR #44, sem merge. Exclusao cross-tab pela Web Locks API durante toda a operacao administrativa; fence IndexedDB como defesa em profundidade, nao como garantia temporal primaria. Sem NUT-004C.
- **Antes:** fence IDB com TTL 30s como unica exclusao entre abas; admin de 31s+ liberava writers no expiry (P1); `renewNutritionAdminFence` renovava fence expirado; verificacoes de expiracao misturavam `Date.now()` com o `now` injetavel; export concorrente com writer terminava em conflito/erro em vez de serializar.
- **Depois:** lock canonico `gymflow:nutrition-admin-v1` (admin EXCLUSIVE, 5 writers SHARED, ordem Web Lock -> fence IDB -> owner-token); capability `navigator.locks` testavel com fail-closed `nutrition-admin-lock-unavailable` nas 6 ops (writers seguem normais); renew de expirado/alheio bloqueado; relogio canonico em todas as expiracoes; `ADMIN_SAFETY_DEPENDS_ON_FENCE_TTL = NO`; 31 testes novos (adapter 23 + provider 8) incl. over-TTL sem sleep, writer-first, multi-aba, throw-release, TOCTOU normal/over-TTL por kind e fallback.
...
## GOAL-094 - GYMFLOW-NUT004C-LIFECYCLE-ROLLOVER-094 lifecycle/rollover nutricional (2026-09-14)

- **Contexto:** slice NUT-004C sobre a base canonica ce06479 (NUT-004A/B INTEGRATED). Somente lifecycle/rollover: dia ativo acompanha a data civil local com app aberto ou ao retornar ao foreground. Sem schema/storage novo, sem backup/restore/reset do ledger, sem NUT-005.
- **Antes:** cold boot reconciliava uma vez (bridge NUT-004B); writes reconciliavam inline duplicado; nenhum resume/foreground (`visibilitychange`/`appStateChange` nutricional inexistentes), nenhum timer — meia-noite com app aberto so era percebida no proximo write ou reload.
- **Depois:** `src/lib/nutrition/lifecycle.ts` com `reconcileNutritionDayForNow` (funcao unica: timezone atual -> targets/gate -> data civil -> ensureToday -> actuals/espelhos) + `NUTRITION_FOREGROUND_RECONCILE_INTERVAL_MS = 60_000`; Provider arma `visibilitychange` nutricional (reusa infraestrutura global, sem segundo sistema), `appStateChange` via import dinamico fail-closed e timer cooperativo so em foreground; `logWater`/`logMacros` reconciliam pela funcao unica antes do mutate; refs so apos persistencia; reconcile nunca concede XP.
- **Arquivos:** `src/lib/nutrition/lifecycle.ts` + `lifecycle.test.ts` (novos), `src/lib/nutrition/index.ts` (barrel), `src/providers/GymFlowContext.tsx` (reconcile unico + efeito lifecycle + refactor writes), `src/providers/GymFlowContext.nutrition-lifecycle.test.tsx` (novo), `docs/DECISOES.md` (D94), `docs/GOALS_LOG.md`.
- **Preservado:** IDB=5, HYBRID=2, CURRENT=1; cold boot/migration/gateSnapshot; admin-lock/fence/export; backup/restore/reset (gate temporario vigente, pendencias NUT004C_LEDGER_ADMIN_DEFERRED/P2/P3 intactas); NUT-005 nao iniciado.
- **Validacao:** focados 15/15 (unit) + 11/11 (Provider); regressoes lib/nutrition+civil-date 349/349 e Provider nutricional 45/45; FULL 3281/3282 com 1 flake sob carga em `storage-retention` (sem relacao com o diff, 45/45 isolado — reportado, sem retry-until-green); `npx tsc --noEmit` 0 erros; `npm run build` OK; `npm run build:mobile` OK; `git diff --check` exit 0.

## GOAL-096 - GYMFLOW-NUT004C-INFLIGHT-CIVIL-DATE-RACE-FIX-096 in-flight keyed por contexto civil (2026-09-14)

- **Contexto:** elimina exclusivamente o P1 do GOAL-095 no mesmo branch do PR #45, sem merge: reconcile iniciada no dia civil D nao pode ser reutilizada por logWater/logMacros ou lifecycle cujo `now` ja pertence a D+1. Sem backup/restore/reset, sem NUT-005.
- **Antes:** `nutritionReconcileInFlightRef` guardava apenas Promise; havendo in-flight, a nova chamada aguardava e reutilizava o resultado mesmo com `now` em outra data civil (CROSS_DAY_INFLIGHT_REUSED = YES — P1).
- **Depois:** in-flight carrega `{ key: `${timezone}:${civilDate}`, promise }` via `resolveEffectiveTimezone` + `getCivilDateString` (nunca UTC); mesma chave reutiliza a Promise (dedup visibility/appState/timer/writes); chave diferente serializa (antigo termina → novo executa com `now`/perfil do novo chamador, entrada nova publicada antes de aguardar, refs/espelhos so com resultado novo); cleanup so se a entrada finalizada ainda for a atual; timezone efetivo distinto = chave distinta; sem profileHash, sem persistencia nova.
- **Arquivos:** `src/providers/GymFlowContext.tsx` (ref do in-flight + `runNutritionReconcile` keyed), `src/providers/GymFlowContext.nutrition-inflight-race.test.tsx` (novo, 7 testes), `docs/GOALS_LOG.md`.
- **Preservado:** backup/restore/reset; Web Locks/fence/admin; cold boot/visibility/appStateChange/timer 60s/background OFF/cleanup/timezone matrix/history; NUT-005 nao iniciado.
- **Validacao:** novo 7/7 (MIDNIGHT water/macros SAFE, visibility/timer/appState D→D+1 PASS, cadeia D→D+1→D+1→D+2, mesmo-dia 10x dedup 1 reconcile); focados lifecycle/rollover/bridge/nutrition 95/95; admin+cross-tab+fence+gate 107/107 (POST_ADMIN divergencia = NO, ADMIN_FLOW_CHANGED = NO); FULL 3289/3289 zero falhas; `npx tsc --noEmit` 0 erros; `npm run build` OK; `npm run build:mobile` OK; `git diff --check` exit 0. INFLIGHT_KEYED_BY_CIVIL_CONTEXT = YES; SAME_DAY_INFLIGHT_DEDUP = YES; CROSS_DAY_INFLIGHT_REUSED = NO; ONE_DAY_PER_CIVIL_DATE = YES; HISTORY_PRESERVED = YES; XP_DUPLICATION = NO; P0 = 0; P1 = 0.

## GOAL-098 - GYMFLOW-NUT004C-LIFECYCLE-FINAL-MERGE-098 merge controlado do PR #45 (2026-09-15)

- **Contexto:** merge controlado do PR #45 em master, encerrando formalmente o lifecycle/rollover NUT-004C ja aprovado nos GOAL-095/096/097. Nenhuma alteracao funcional adicional.
- **Pre-flight:** PR #45 OPEN; headRefOid `5da5f36daf5219d2ffd39956094495db9caaad62`; origin/master `ce06479085df9a61a2e13bac901018d23ca442d4`; mergeable MERGEABLE; mergeStateStatus CLEAN; commits = 2.
- **CI no HEAD aprovado:** iOS Cloud Runtime Validation SUCCESS; iOS Distribution Readiness Validation SUCCESS; zero checks running/failing.
- **Evidencia herdada (sem repetir auditoria completa):** GOAL-095 P1 cross-day reproduzido; GOAL-096 P1 corrigido; GOAL-097 NUT004C_INFLIGHT_FINAL_REVIEW = APPROVED, READY_FOR_MERGE_GATE = YES, P0 = 0, P1 = 0; FULL 3289/3289; TYPECHECK PASS; BUILD PASS; MOBILE_BUILD PASS; DIFF_CHECK PASS.
- **Merge:** merge commit `fa8c595d46cfd450bd5769fba3387a750d108f64` (metodo merge, sem squash/rebase/admin/force); 2 commits do PR preservados; pais `ce064790...` + `5da5f36d...`; HEAD aprovado na ancestralidade da nova master; zero commits extras entre pre-flight e merge.
- **Status:** NUT004A_STATUS=INTEGRATED; NUT004B_STATUS=INTEGRATED; NUT004C_LIFECYCLE_STATUS=INTEGRATED. Fechados nesta etapa: cold boot preservado, visibilitychange, Capacitor appStateChange, timer cooperativo 60s, background timer OFF, rollover por data civil, timezone correto, write pos-meia-noite, dedupe same-day, serializacao cross-day, cleanup/failure recovery do in-flight, history preservation, XP sem duplicacao.
- **Preservado fora:** backup logico completo do ledger, restore completo do ledger, reset completo do ledger, remocao do admin gate temporario, NUT-005 (nao iniciados).
- **Validacao:** docs-only (markdown); `git diff --check` OK; arvore de codigo byte-identica a validada pela CI no merge.

## GOAL-100 - GYMFLOW-NUT004C-LEDGER-ADMIN-IMPLEMENTATION-100 ledger-aware (2026-09-15)

- **Contexto:** ultima fronteira estrutural do NUT-004 sobre a base canonica 73d9743 (NUT-004A/B + lifecycle INTEGRATED, auditoria GOAL-099 como contrato). Backup logico schema 2 com section nutritionLedger obrigatoria; import/restore/reset com journal nutricional, verificacao cruzada core-ledger e recovery no boot; remocao do gate temporario. Sem NUT-005, sem merge em master.
- **Antes:** backup schema 1 sem ledger (payload = core+historico); import/restore/reset tocavam so core/historico; Provider bloqueava as seis operacoes com nutrition-ledger-admin-deferred quando havia consumo real (gate TOCTOU-fechado por fence+Web Lock, mas sem snapshot do ledger); receipts sem fase nutricional; boot recovery ignorava o ledger (risco de hibrido core-novo+ledger-antigo).
- **Depois:** envelope schema 2 com 9 chaves (nutritionLedger: days/activeDate/migrationMarker, sempre presente inclusive days:[]); digest payloadDigest cobre payload+ledger; snapshot IDB readonly consistente (days+activeDate+marker, sem fence, IDB v5 sem store novo); import valida tudo antes do write (schema1+ativo => legacy-backup-with-active-ledger, sem reload/wipe) e segue journal begin -> stage dias -> core -> metadata -> verify cruzado -> settled -> reload; restore usa previous-ledger do receipt-fonte (legado+ativo => fail-closed); reset seletivo zera days/activeDate/marker + core zerado preservando o fence; receipts com previous/target raws + digest/activeDate/marker + status; boot recovery converge o ledger ao mesmo mundo do core sob EXCLUSIVE; espelhos derivam do ledger no cold boot (sem XP); gate removido (locks+fence permanentes); EMPTY_LEDGER_PASS_THROUGH eliminado.
- **Arquivos:** novos src/lib/storage-nutrition-ledger-backup.ts + src/lib/storage-nutrition-ledger-admin.test.ts (12 provas); backup/import/restore/reset/receipt/admin-runtime/indexeddb/administrative-recovery ledger-aware; Provider sem as seis sondas (reasons legacy-backup-with-active-ledger, UI sem deferred); testes de gate/fence/cross-tab atualizados; allowlists de call-site atualizadas; docs/DECISOES.md (D100) + docs/PENDENCIAS.md (DEFERRED resolvido, PASS_THROUGH eliminado).
- **Preservado:** IDB=5, HYBRID=2, CURRENT=1, BACKUP_FORMAT=2; formato fisico e stores intactos; dominio puro de nutricao intocado (reuso de isNutritionDay/isLedgerMigrationMarker/gateSnapshot); cold boot/migration/lifecycle/rollover/XP/civil-date inalterados; NUT-005 nao iniciado.
- **Validacao:** focados backup/import/restore/reset/recovery + idb-nutrition + admin lock/fence/gate + bridge/lifecycle/rollover/ledger/migration + 12 provas GOAL-100 verdes; FULL 3301/3301 zero falhas; tsc 0 erros; build OK; build:mobile OK; diff check OK; P0=0 P1=0.

## GOAL-104 - GYMFLOW-NUT004C-LEDGER-ADMIN-FINAL-MERGE-104 merge controlado do PR #46 (2026-09-16)

- **Contexto:** merge controlado do PR #46 em master, encerrando formalmente o ledger-admin NUT-004C ja aprovado nos GOAL-101/102/103. Nenhuma alteracao funcional adicional.
- **Pre-flight:** PR #46 OPEN; headRefOid `7736a8fbdff665ec20ba093053271800eaf1d68c`; origin/master `73d9743051f60e9efa508096b1b90395ae3d1d3b`; mergeable MERGEABLE; mergeStateStatus CLEAN; commits = 2 (`7fde086` GOAL-100 + `7736a8f` GOAL-102).
- **CI no HEAD aprovado:** iOS Cloud Runtime Validation SUCCESS; iOS Distribution Readiness Validation SUCCESS; zero checks running/failing.
- **Evidencia herdada (sem repetir auditoria completa):** GOAL-101 P1 recovery sem Web Lock encontrado; GOAL-102 fallback inseguro removido, recovery sem lock = fail-closed, journal preservado, replay posterior = PASS; GOAL-103 NUT004C_LEDGER_RECOVERY_FINAL_REVIEW = APPROVED, READY_FOR_MERGE_GATE = YES, P0 = 0, P1 = 0; FULL 3308/3308; TYPECHECK PASS; BUILD PASS; MOBILE_BUILD PASS; DIFF_CHECK PASS.
- **Merge:** merge commit `42b81e0fbc6c5a86ebdf692c0315c824884f944e` (metodo merge, sem squash/rebase/admin/force); 2 commits do PR preservados; pais `73d9743...` + `7736a8f...`; HEAD aprovado na ancestralidade da nova master; zero commits extras entre pre-flight e merge.
- **Status:** NUT004A_STATUS=INTEGRATED; NUT004B_STATUS=INTEGRATED; NUT004C_LIFECYCLE_STATUS=INTEGRATED; NUT004C_LEDGER_ADMIN_STATUS=INTEGRATED. Fechados nesta etapa: backup schema 2 ledger-aware, digest cobrindo ledger, import ledger-aware, restore ledger-aware, reset seletivo, journal/recovery core+ledger, recovery fail-closed sem Web Lock, replay posterior, fence nao restaurado, admin gate removido, EMPTY_LEDGER_PASS_THROUGH eliminado.
- **Preservado fora:** homologacao final do NUT-004 e NUT-005 (nao iniciados); pendencias nao bloqueantes mantidas sem reclassificacao (LEGACY_LOGGED_AT_APPROXIMATION P2, LEGACY_WATER_NO_CEILING P2, DIAS_083_SEM_GATE_REJEITADOS P3).
- **Validacao:** docs-only (markdown); `git diff --check` OK; arvore de codigo byte-identica a validada pela CI no merge.

## GOAL-106 - GYMFLOW-NUT005-FOOD-DATABASE-SEARCH-ENDTOEND-106 implementacao (2026-09-16)

- **Base:** origin/master `91d908d5f985acc692e4d83477db662842bcb663` (confere com a base esperada; sem avanco antes do inicio); branch `feat/gymflow-nut005-food-database-106`.
- **Antes:** sem `FoodDatabase` (divida P2 do Masterplan 3.1): sem catalogo local, sem busca, sem cadastro manual validado, sem favoritos/recentes, sem ponte FoodReference->FoodEntry; ledger NUT-004 (A/B/C lifecycle/admin) verde e preservado.
- **Depois:** `src/lib/nutrition/food-types.ts` (FoodSource V1 exata, FoodReference + `sourceRef` auditavel, teoria 4P+4C+9G, USDA_FDC offline com FDC ID preservado, USER_CONFIRMED com ACCEPTED/NEEDS_CONFIRMATION/REJECTED e confirmacao explicita); `src/lib/nutrition/food-database.ts` (catalogo validado/congelado, busca em memoria case+accent-insensitive com ranking exato>prefixo>substring>brand, escala de porcao em etapa unica, `toFoodEntryInput` sem escrita no ledger); `src/lib/nutrition/food-preferences.ts` (favoritos/recentes por IDs em chave/valor, limite 30, sem IDB novo); `src/data/canonical-br-foods.json` (160 CANONICAL_BR, base USDA SR Legacy dominio publico, proxies/harmonizacoes declarados, zero TBCA/OFF/barcode); `food-database.test.ts` + `food-preferences.test.ts` (47 testes).
...
## GOAL-107 - GYMFLOW-NUT006-MOBILE-UX-ENDTOEND-107 implementacao (2026-09-16)

- **Base:** origin/master ab95ff30e2506941c0c0d79f44710dbff6dc8980 (confere com a base esperada; sem avanco antes do inicio); branch feat/gymflow-nut006-mobile-ux-107.
- **Antes:** NutritionPage em UX legada com formulario manual de macros e sugestoes estaticas hardcoded; Provider expunha so espelhos + logWater/logMacros, sem acesso reativo ao dia ativo, historico ou favoritos.
- **Depois:** Provider com superficie minima real (nutritionDay/activeDate/timezone/targetState/loading/error/favorites/recents + logFoodReference/getNutritionHistory/refreshNutrition/toggleNutritionFavorite; logWater/logMacros preservados); NutritionPage mobile-first em 5 abas (Hoje/Registrar/Metas/Tendencia/Sugestoes); src/components/nutrition/** (Today/Register/Targets/Trend/Suggestions + shared); src/lib/nutrition/suggestions.ts + trend.ts puras e deterministicas; USER_CONFIRMED reutiliza validacao 15% do NUT-005 com confirmacao explicita em 2 etapas; nenhuma selecao grava automaticamente; sem rotulo de IA; BottomNavigation inalterada.
- **Arquivos:** src/providers/GymFlowContext.tsx, src/modules/NutritionPage.tsx, src/components/nutrition/shared.tsx|TodaySection.tsx|RegisterSection.tsx|TargetsSection.tsx|TrendSection.tsx|SuggestionsSection.tsx, src/lib/nutrition/suggestions.ts|trend.ts|index.ts, testes focados (suggestions|trend|NutritionPage|provider-nut006, 17 testes), docs/DECISOES.md (D107-001/D107-002).
- **Validacoes:** focados NUT-006 17/17; suite completa 146 arquivos 3367/3367 (zero regressao NUT-004/005/ledger/rollover/midnight/FoodDatabase/favorites-USER_CONFIRMED/targets-gates/XP/backup); npx tsc --noEmit 0 erros; npm run build OK; npm run build:mobile OK; git diff --check limpo; QA: prod start 200 OK + 20/20 botoes >=44px + sem alert-confirm + sem rotulo IA (dev-server Turbopack indisponivel no ambiente, ver D107-002).
- **Status:** NUT-006 implementado e validado nesta branch (INTEGRATED apos merge do PR); NUT-007/OFF-barcode nao iniciados.

## GOAL-107-MERGE - GYMFLOW-NUT006-FINAL-MERGE-107 merge controlado do PR #48 (2026-09-16)

- **Pre-flight:** PR #48 OPEN; headRefOid e5d47e4468758755b8520c0862145a52b899b9e8; origin/master ab95ff30e2506941c0c0d79f44710dbff6dc8980 (confere com a base esperada, sem avanco); mergeable MERGEABLE; mergeStateStatus CLEAN.
- **CI no HEAD aprovada:** Build & Simulator Validation (macOS 26) pass 6m28s; Release Archive & Distribution Gate (macOS 26) pass 3m42s; zero checks failing/pending.
- **Merge:** merge commit cb5f31b9558e4551d845f75e29b870dcdf0d1b76 (metodo merge, sem squash/rebase/admin/force); pais ab95ff3... + e5d47e4...; HEAD aprovado na ancestralidade da nova master.
- **Status:** NUT004_REGRESSION=NONE; NUT005_REGRESSION=NONE; NUT006_STATUS=INTEGRATED; READY_FOR_NUT007=YES. Fechados nesta entrega: 5 secoes mobile-first sobre ledger real, registro real por FoodReference com foodReferenceId, busca+favoritos+recentes, USER_CONFIRMED 15% com confirmacao explicita, targets AUTOMATED/MANUAL_ONLY honestos, tendencia 7/30 sem interpolacao, sugestoes deterministicas sem IA, hidratacao +250/+500/+1L. Nao iniciados: NUT-007, NUT-008, OFF/barcode.
- **Validacao:** docs-only (markdown); git diff --check OK; arvore de codigo byte-identica a validada pela CI no merge.

## GOAL-108 - GYMFLOW-NUT007-CONTROLLED-AI-NUTRITION-ENDTOEND-108 implementacao (2026-09-16)

- **Base:** origin/master c98a58e030b29a47db4ef4269fcf5a430ee72645 (confere com a base esperada; sem avanco antes do inicio); branch feat/gymflow-nut007-ai-assistant-108.
- **Antes:** sem assistente IA (NUT004/005/006 INTEGRATED, READY_FOR_NUT007=YES); auditoria confirma zero infra de IA no projeto/deploy (nenhum GYMFLOW_AI_/OPENAI em src; training-plan-assistant 100% offline).
- **Depois:** ai-assistant-types.ts (5 casos canonicos tipados, limites estritos, AiAssistantError, estados da UI); ai-assistant.ts (nucleo puro read-only: validacao estrita, grounding com recalculado via scaleFoodReferenceToGrams, macros do modelo ignorados, prompt com texto do usuario so como DADOS, deteccao de injection, allowlist deterministica); ai-assistant-provider.ts (server-only OpenAI-compatible via fetch sem SDK, timeout/cancel, teto de bytes); ai-assistant-gateway.ts (pipeline 413/400/409/403/503/502/504/200 com gates antes do modelo e filtro de saldo nos casos 1 e 4); route.ts POST same-origin (POST-only sem force-dynamic para preservar o export mobile); ai-assistant-client.ts (web same-origin, mobile so backend HTTPS por origem publica, AI_UNAVAILABLE honesto); AiMealAssistantModal.tsx (8 estados, CTA Adicionar ao Diario em 2 etapas via logFoodReference do NUT-006, sem auto-write) + botao de abertura na SuggestionsSection (offline intacto) + nutritionProfile somente-leitura no Provider.
- **Arquivos:** src/lib/nutrition/ai-assistant-{types,,provider,gateway,client}.ts(+.test), src/app/api/nutrition/assistant/route.ts, src/components/nutrition/AiMealAssistantModal.{tsx,test.tsx}, src/components/nutrition/SuggestionsSection.tsx, src/lib/nutrition/index.ts, src/providers/GymFlowContext.tsx, docs/DECISOES.md, docs/GOALS_LOG.md, docs/PENDENCIAS.md.
- **Validacoes:** focados NUT-007 53/53 (5 casos, ID inexistente, macros falsos, malformado, injection, MANUAL_ONLY, gate bloqueado, provider ausente, timeout, HTTP erro, nao-JSON, duplicacao, gramagem invalida, confirmacao 2 etapas, permissoes read-only); suite completa 150 arquivos 3420/3420 (3367 herdados + 53 novos, zero regressao NUT-004/005/006); npx tsc --noEmit 0 erros; npm run build OK (rota /api/nutrition/assistant dinamica); npm run build:mobile OK (POST-only fora do out/, client mobile via backend remoto); git diff --check limpo; smoke web real: HOME 200 + gateway 503 AI_UNAVAILABLE/UNCONFIGURED + 409 MANUAL_ONLY + 400 malformado; bundles: .next/static e out/ com zero GYMFLOW_AI_API_KEY/chat-completions (so NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL = origem publica, permitido); mobile out/ com modal + estados honestos + sugestoes offline.
- **Runtime gate:** AI_RUNTIME_PROVIDER = UNCONFIGURED (so VERCEL_OIDC_TOKEN no .env.local, nenhuma credencial GYMFLOW_AI_* disponivel; nenhuma credencial inventada) - blocker operacional legitimo; READY_FOR_NUT008=NO ate smoke real com provedor.
- **Status:** NUT007 implementado e validado nesta branch (INTEGRATED apos merge do PR, condicionado ao runtime gate acima); NUT-008/OFF-barcode nao iniciados. P0=0 P1=0.

## GOAL-108-MERGE - GYMFLOW-NUT007-FINAL-MERGE-108 merge controlado do PR #49 (2026-09-16)

- **Pre-flight:** PR #49 OPEN; headRefOid ba8139988600c1464791cd8ad141610ce0bd1c60; origin/master c98a58e030b29a47db4ef4269fcf5a430ee72645 (confere com a base esperada, sem avanco); mergeable MERGEABLE; mergeStateStatus CLEAN.
- **CI no HEAD aprovada:** Build & Simulator Validation (macOS 26) pass 11m13s; Release Archive & Distribution Gate (macOS 26) pass 2m27s; zero checks failing/pending.
- **Merge:** merge commit c3d1af0503dd00da23b56e1e06f7e985f54ee79a (metodo merge, sem squash/rebase/admin/force); pais c98a58e... + ba81399...; HEAD aprovado na ancestralidade da nova master; arvore do merge byte-identica a validada pela CI.
- **Status:** NUT004_REGRESSION=NONE; NUT005_REGRESSION=NONE; NUT006_REGRESSION=NONE; NUT007_STATUS=INTEGRATED (implementacao pronta e validada; runtime provider UNCONFIGURED como blocker operacional legitimo); READY_FOR_NUT008=NO (vira YES apos smoke real com GYMFLOW_AI_* configurado). Nao iniciados: NUT-008, OFF/barcode.
- **Validacao:** docs-only (markdown); git diff --check OK.

## GOAL-109 - GYMFLOW-NUT007-AI-RUNTIME-ACTIVATION-ENDTOEND-109 tentativa de ativacao do runtime real (2026-09-16)

- **Base:** origin/master 649772f7ca16846e82de09c223febe18b3d6eadf (HEAD local identico; tracked limpo, so untracked pre-existentes).
- **Auditoria de deployment (somente nomes, nenhum valor impresso/logado/copiado):** projeto Vercel `gymflow` ligado, sem nenhum deployment (Latest Production URL vazio); `vercel env ls` lista apenas BLOB_READ_WRITE_TOKEN (Production, Preview); `.env.local` contem apenas VERCEL_OIDC_TOKEN; shell sem nenhuma variavel GYMFLOW_AI_/OPENAI/ANTHROPIC/GEMINI/GROQ/MISTRAL/DEEPSEEK/HUGGING; C:\Projetos\secrets sem nenhuma chave GYMFLOW_AI_* (5 arquivos verificados por nome de chave, sem leitura de valores).
- **Decisao provider (§2/§4 do GOAL):** nenhuma credencial utilizavel em fonte segura; nenhum provider pre-existente para reutilizar; nenhuma troca por preferencia. PARADA sem inventar chave — blocker externo legitimo. Variaveis faltantes exatas: GYMFLOW_AI_ENABLED, GYMFLOW_AI_BASE_URL, GYMFLOW_AI_API_KEY, GYMFLOW_AI_MODEL (e NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL quando o Capacitor for ligado a um backend).
- **Sem configuracao (§4):** nada escrito em fonte segura pelo executor; nenhum segredo movido para arquivo versionado; nenhum .env com segredo criado para commit. Sem deploy (§5 — exige runtime configurado; plataforma sem deployment previo do projeto gymflow para reutilizar). Sem smoke real (§6 — exige gateway com provedor; path honesto UNCONFIGURED revalidado pelos testes de gateway).
- **Validacoes sem mudanca de codigo (§12):** focados NUT-007 53/53 (ai-assistant 50/50 em 3 arquivos + AiMealAssistantModal 3/3); suite completa nao repetida; tsc/build nao repetidos (arvore de codigo byte-identica a validada na CI do merge c3d1af0). git diff tracked vazio; git diff --check limpo.
- **Secret scan (§10):** `GYMFLOW_AI_API_KEY` = 6 refs em src restritas ao NOME da env (leituras server-only, comentarios, stub de teste com placeholder literal `test-key-sem-valor-real`); 0 ocorrencias em .next/static; 0 em out/; 0 `chat/completions` em .next/static e out/; 0 Bearer real; 0 NEXT_PUBLIC_* com KEY/SECRET/TOKEN em src. SECRET_EXPOSURE = NO.
- **Correcoes (§11):** nenhum bug revelado (smoke real nao executavel por falta de credencial; path honesto intacto). Nenhum branch/PR criado; nenhum codigo alterado.
- **Aceite parcial honesto:** AI_RUNTIME_PROVIDER = UNCONFIGURED; REAL_PROVIDER_SMOKE = NOT_RUN (BLOCKED_EXTERNAL_CREDENTIAL); USE_CASE_1..5 = NOT_RUN; CATALOG_GROUNDING/MODEL_MACROS_TRUSTED/AI_CAN_WRITE_* = preservados por codigo inalterado, nao re-homens homologados em runtime; WEB_RUNTIME = HONEST_UNAVAILABLE (fail-closed verificado em teste); MOBILE_BACKEND_ROUTE = NOT_RUN (sem backend); P0 = 0; P1 = 0.
- **Status:** NUT007_RUNTIME_STATUS = BLOCKED_EXTERNAL_CREDENTIAL; NUT007_STATUS = INTEGRATED (inalterado); READY_FOR_NUT008 = NO. Nao iniciados: NUT-008, OFF/barcode.
- **Validacao:** docs-only (markdown); git diff --check OK.

## GOAL-110 - GYMFLOW-NUT007-OPENROUTER-RUNTIME-VALIDATION-ENDTOEND-110 homologacao runtime (2026-09-16)

- **Base / retomada:** origin/master = HEAD = 3f1ed1f256be463f57546867da97e20eea33444c (fechamento docs-only do GOAL-109). Sessao anterior interrompida por falha de shell/CLR; checkpoint reconstruido so com Git/Vercel/deployments/envs existentes. Nenhum restart do zero.
- **Checkpoint Vercel:** projeto linkado `gymflow` (prj_xgWgYaxraom2GlyM5rcprGdAeJfe). 4 deployments Production READY; mais recente `dpl_87GsiQpvzhX2V6brNZft3U7TRVvh` alias https://gymflow-beige-gamma.vercel.app, source=cli, gitCommitSha=3f1ed1f (DEPLOY_SHA = BASE_SHA). Envs Production (somente nomes): GYMFLOW_AI_ENABLED, GYMFLOW_AI_BASE_URL, GYMFLOW_AI_API_KEY, GYMFLOW_AI_MODEL, NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL (+ BLOB_READ_WRITE_TOKEN herdado). Nenhuma env recriada/sobrescrita. Valor de GYMFLOW_AI_API_KEY nunca lido/impresso nesta sessao.
- **Provider:** OpenRouter via adapter OpenAI-compatible existente. Smoke real ~1.6–3.0s por caso (nao mock). GYMFLOW_AI_MODEL inalterado (server-only, tipo sensitive na Vercel; nao aparece no bundle). NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL comprovado no chunk de producao como origem `https://gymflow-beige-gamma.vercel.app` (client nativo concatena `/api/nutrition/assistant`).
- **Smoke 5 casos (POST /api/nutrition/assistant em producao):** USE_CASE_1 complete_protein 200 ok (br-peito-frango-grelhado 120g + br-queijo-cottage 100g; totais locais 286.8 kcal / 49.4 P); USE_CASE_2 substitute_food 200 ok (replace br-arroz-branco-cozido → br-queijo-cottage; IDs de catalogo); USE_CASE_3 build_meal_from_ingredients 200 ok (asa/arroz/brocolis; unresolved=[]); USE_CASE_4 snacks_within_balance 200 ok (3 opcoes: iogurte, cottage, atum; todas <=800 kcal); USE_CASE_5 explain_target_change 200 ok (facts ecoados 2500/160/300/70; explanationLen=226). IDs FoodReference validos; macros `computed` locais (ex.: 120g peito = 190.8 kcal = 159*1.2, identico ao catalogo do teste 150g=238.5).
- **Negativos producao:** MANUAL_ONLY 409; CLINICAL_GATE_BLOCKED 403 (mensagem: modelo nao chamado); payload useCase invalido 400 INVALID_REQUEST; JSON malformado 400; FoodReference inexistente no substitute 400 (rejeicao no catalogo antes do modelo); REQUEST_TOO_LARGE 413; GET 405; abort de cliente AbortError. INVALID_RESPONSE/timeout de provedor cobertos pelos testes de gateway (fetch injetado) sem forcar timeout de 12s contra o provider real.
- **Secret audit:** HOME 200 24515 bytes. Bundles de producao e artefato mobile `out/`: 0 GYMFLOW_AI_API_KEY, 0 chat/completions, 0 openrouter.ai, 0 Bearer real, 0 NEXT_PUBLIC_*KEY/SECRET/TOKEN. `out/api` ausente. Capacitor aponta so origem publica GymFlow; gateway server-side chama o provider.
- **Mobile:** `npm run build:mobile` OK com origem publica GymFlow; client resolve remote `https://gymflow-beige-gamma.vercel.app/api/nutrition/assistant`. Harness = o mesmo POST real do smoke (identico ao caminho nativo). Focados NUT-007 53/53 (2 falhas transientes so com env de build vazada no shell; 10/10 no client apos limpar NEXT_PUBLIC_*). Suite completa/CI nao repetidas (sem mudanca de codigo).
...
## GOAL-111-MERGE - GYMFLOW-NUT008-INTEGRATED-QA-BETA-READINESS-ENDTOEND-111 merge controlado do PR #50 (2026-09-17)

- **Pre-flight:** PR #50 OPEN ready; headRefOid 7aab83e5b4be57e82a7e3e570d81dded2f5800b3; origin/master a4fbf15d267c4fa9efcafe5756c8deb4d394c541 (confere com a base esperada, sem avanco); mergeable MERGEABLE; mergeStateStatus CLEAN.
...
## GOAL-115 - GYMFLOW-ANDROID-INTERNAL-RELEASE-READINESS-ENDTOEND-115 prontidao release interno Android (2026-09-18)

- **Base:** origin/master 1698e55b8cce0119bc4dd3286cdb3f3d063f7c0b (confere com a base esperada); branch feat/gymflow-android-internal-release-115.
- **Antes:** packageId com.gymflowai.app, versionCode 1 / versionName 1.0, AGP 8.7.2 / Gradle 8.11.1 / JDK 17, signing so via template (AAB existente unsigned, APK so debug), sem script de APK release, webContentsDebuggingEnabled true, sem workflow Android, sem guia operacional de release interno.
- **Depois:** scripts android-apk-release.mjs (assembleRelease) e android-generate-release-key.mjs (chave interna git-ignorada, PKCS12 storepass=keypass, senhas nunca impressas); webContentsDebuggingEnabled false por padrao; workflow android-internal-readiness.yml (PR: tsc + gate NUT-008 + build:mobile + cap sync + assembleRelease/bundleRelease unsigned + scan); guia docs/mobile/ANDROID_INTERNAL_RELEASE_115.md (release, signing, secrets, versionamento, smoke, pre-upload Internal Testing); secao release em docs/ANDROID_BUILD.md; versao mantida 1/1.0 com regra minima (+1 versionCode por upload).
- **Artefatos release assinados (chave interna gymflow-internal, git-ignorada):** android/app/build/outputs/apk/release/app-release.apk (28117129 bytes) e android/app/build/outputs/bundle/release/app-release.aab (27834716 bytes, 822 entradas, 0 .so, 0 .mp4); apksigner DN CN=GymFlow Internal (nao-debug), jarsigner verified; aapt2 package com.gymflowai.app versionCode 1 versionName 1.0 minSdk 23 targetSdk 36 compileSdk 36; permissoes so INTERNET + DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION.
- **Backend/secrets:** release usa somente NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL (bundle sem valor embutido = unavailable honesto ate configurar por release); Production https://gymflow-beige-gamma.vercel.app validado (POST invalido 400 fail-closed, GET 405 POST-only); scan APK+AAB web bundles (32 arquivos cada): 0 GYMFLOW_AI_API_KEY, 0 openrouter.ai, 0 chat/completions; localhost/open achados = parser de URL do Next e expressao de leitura do env (benignos). OPENROUTER_API_KEY_EXPOSURE=NO, SERVER_SECRET_EXPOSURE=NO, DIRECT_PROVIDER_CALL=NO.
...
## GOAL-117 - GYMFLOW-ANDROID-PLAY-INTERNAL-ONBOARDING-ENDTOEND-117 onboarding Google Play Internal Testing (2026-09-24/25)

- **Base:** origin/master 5619d27275292484c34be4c6b25802e077514d3e (confere com a base esperada); branch feat/gymflow-android-play-internal-onboarding-117; PR #53.
- **Antes:** unica chave = interna gymflow-internal (CN=GymFlow Internal, SHA-256 A3:27:3A:11:...:DE:54:7C) em android/release-signing.properties; APK/AAB existentes assinados por ela (inelegiveis para o Play); Gradle misturava env e properties campo a campo; build:mobile sem trava de backend; sem auditoria de assinatura/identidade do AAB; estado do Play Console desconhecido (GOAL-020: conta ainda nao existia).
- **Depois:** android-generate-upload-key.mjs (upload key PKCS12 RSA 4096 SHA256withRSA 10000 dias, --out-dir obrigatorio fora de qualquer repo git com symlink/junction resolvidos, senha sem eco via env do filho, sem sobrescrita, registro publico android/play-upload-certificate.json); android-play-release.mjs (confere fingerprint antes de compilar, --expect-version-code, arvore limpa antes/depois do cap sync, modo de backend explicito e exclusivo conferido contra a origem efetiva, gradle --no-daemon, auditoria); android-release-audit.mjs (apksigner/jarsigner -verbose/keytool: toda entrada assinada + signer unico == upload cert; identidade APK+AAB via manifest proto/aapt2; web do AAB identico a out/ arquivo a arquivo; scan de segredos/URLs de dev no AAB inteiro; backend lido do resolvedor compilado com vinculo por identificador; git sem keystore/senha literal; manifest com hashes); build.gradle com credenciais atomicas; build:mobile valida a origem EFETIVA via @next/env (devDependency exata 16.2.6) e so aceita Production ou nenhuma; runbook docs/mobile/ANDROID_PLAY_INTERNAL_ONBOARDING_117.md com gate humano granular (criacao do app/conta/FREE-PAID, Play App Signing, upload, lancamento).
- **Achado P1 (aberto, decisao humana):** release Android nativo NAO aponta para backend: NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL nunca definida no build mobile e gateway Production sem CORS (OPTIONS com Origin https://localhost -> 204 sem Access-Control-Allow-Origin). IA nativa = unavailable honesto. AI_BACKEND_SMOKE=PASS do GOAL-116 foi curl do shell do aparelho, nao chamada do app. Correcao = GOAL de backend autorizado (CORS + deploy) -> build --expect-backend-production -> novo versionCode.
- **Revisao independente:** Codex CLI GPT-5.6-Sol (familia OpenAI), conteudo inline (sandbox read-only do Codex no Windows bloqueia leitura; 1a tentativa recusou opinar sem ler). 8 rodadas: R1 P0=0/P1=5/P2=2; R2 0/3/3; R3 0/2/3; R4 1/1/2 (N6 P0: .env* ignorado podia embutir backend arbitrario); R5 0/2/3; R6 0/1/2; R7 0/2/1; R8 (2757b34) P0=0/P1=1/P2=1. Todos corrigidos no GOAL exceto N1 (backend, por desenho); N13 (P2) corrigido apos R8 com testes.
- **Validacoes:** npm test 159 arquivos / 3529+ testes PASS (72 do ferramental); npx tsc --noEmit 0 erros; npm run build OK; git diff --check limpo; 5 E2E completos com chave DESCARTAVEL (subject THROWAWAY TEST ONLY; apagada apos cada rodada): ultimo sobre 473b440 BUILD SUCCESSFUL, 36 gates PASS, 0 FAIL, 2 WARN esperados, senha 0x em log/manifest; negativos (out-dir no repo, junction para o repo, senha fraca, sem TTY, sobrescrita, senha errada, versionCode divergente, keystore no repo, arvore suja, --allow-dirty com cert nao descartavel ou registro adulterado, env Gradle parcial, backend LAN/nao-Production, .env com origem estrangeira, modos de backend ausentes/ambos/divergentes da origem efetiva, --skip-web) todos recusados; adulteracao (entrada nao assinada injetada no AAB, out/ divergente) reprovada; builds reais nos 3 modos de backend (sem origem, Production, estrangeira via excecao de QA) classificados corretamente.
- **Status:** UPLOAD_KEY=PENDING_HUMAN; UPLOAD_KEY_BACKUP=HUMAN_KEY_BACKUP_DECISION_REQUIRED; UPLOAD_CERT=PENDING; AAB_RELEASE=PENDING; AAB_SIGNING=PENDING; SECRET_EXPOSURE=NO (baseline); BACKEND_PRODUCTION=FAIL (NOT_CONFIGURED + CORS ausente, P1 aberto); PLAY_APP_SIGNING=BLOCKED_HUMAN_GATE; INTERNAL_TESTING_UPLOAD=BLOCKED_HUMAN_GATE; PLAY_INTERNAL_INSTALL_QA=NOT_YET_AVAILABLE; PUBLIC_BETA_PUBLISHED=NO; PLAY_PRODUCTION_PUBLISHED=NO; D-NUT-08/09 inalterados (PENDING). Nenhuma acao no Google Play. Nao iniciados: OFF/barcode.

## GOAL-118 - GYMFLOW-MOBILE-AI-BACKEND-PRODUCTION-BRIDGE-118 Assistente IA nativo via backend Production (2026-09-25)

- **Base:** origin/master 0679e200fcfff699aced298b43011a42e8cba826 (confere com a base esperada); branch feat/gymflow-mobile-ai-backend-production-bridge-118.
- **Antes (comportamento critico):** app nativo sem backend de IA: NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL nunca entrava no build mobile (IA nativa = unavailable honesto); gateway Production sem CORS (OPTIONS com Origin https://localhost -> 204 sem Access-Control-Allow-Origin; POST de qualquer origem processado); smoke do GOAL-116 foi curl do shell do aparelho.
- **Depois:** gateway com CORS cirurgico so para https://localhost e capacitor://localhost (origem ecoada, nunca *; preflight so POST + Content-Type; CORS em 200/400/403/409/413/502/503/504; Vary), web same-origin e chamadas sem Origin inalteradas, qualquer outra origem -> 403 antes de ler o corpo (provedor nunca chamado); build:mobile com --ai-backend production|none (padrao production, constante versionada, .env so pode repetir a Production); android:play:release passa o modo aprovado ao build:mobile. Deploys Production (sempre de worktree limpo do commit, alias https://gymflow-beige-gamma.vercel.app): dpl_9b1V1yrYdvSEUYAn5xoPXjReq2rb (3ef25b6), dpl_Ccd5u2tzGinRNphqJLY9TLWy5Hjh (8484d96) e o atual dpl_FmmXkoZuu75tgBA3oGatsvtkeJ2b (b8cfc81), todos READY. Runbook/evidencias: docs/mobile/ANDROID_AI_BACKEND_BRIDGE_118.md.
- **Production:** OPTIONS Android/iOS 204 com ACAO exato; origem estrangeira/LAN/outro host Vercel/null/metodo PUT/header Authorization -> 403 sem CORS; POST Android/iOS complete_protein e substitute_food 200 grounded com CORS; 400/403/409/413 com CORS; same-origin 200 sem CORS (curl e navegador real); pagina em https://example.com bloqueada pelo navegador real.
- **Android (chave interna, sem upload key):** build:mobile -> cap sync -> gradlew clean assembleRelease bundleRelease -> android:release:audit --no-record --expect-backend-production = 36 PASS / 0 FAIL / 0 WARN em cada HEAD (3ef25b6, 8484d96, b8cfc81; BACKEND_PRODUCTION_ORIGIN_EMBEDDED, BACKEND_MODE_MATCHES_APPROVAL, scans de segredo/dev em APK/AAB/out/). Final b8cfc81: APK a387363d...; AAB 1b58cc14...; versionCode 1 / versionName 1.0.
- **Smoke nativo (emulador Pixel_6_API_35, API 35; S22 nao conectado):** caso bloqueado PROFILE_ABSENT -> "a IA nao foi chamada" (client nao chama o gateway); apos restore pelo app de backup v2 com perfil adulto: complete_protein (frango 120 g = 190,8 kcal) e substitute_food (arroz 100 g -> iogurte 170 g, delta -62 kcal) renderizados; texto do modelo variando com macros identicos; "Adicionar ao Diario" exige confirmacao; 0 kcal gravados automaticamente; metas inalteradas; offline -> AI_UNAVAILABLE honesto; online -> IA volta; ledger 500 ml / 2 registros preservado apos kill/reopen; 0 crash / 0 ANR / 0 erro CORS; captura de rede: TLS SNI gymflow-beige-gamma.vercel.app, 0 openrouter. Ressalva: no Wi-Fi virtual (beacon loss) 4/5 primeiras chamadas apos troca de rede expiraram no timeout do client (honesto); celular 3/3 OK (pendencia P2 para o S22).
- **Segredos:** out/, APK, AAB e chunks servidos pela Production: 0 GYMFLOW_AI_API_KEY, 0 openrouter.ai, 0 chat/completions, 0 Bearer real, 0 host dev/estrangeiro. DIRECT_OPENROUTER_CLIENT_CALL=NO; SECRET_EXPOSURE=NO.
- **Privacidade:** Data Safety (005 §9), App Privacy iOS (006 §3.3), dossie D-NUT-09 §4/§5/§7 e checklist juridico atualizados com os fatos do fluxo nativo (app -> gateway Vercel iad1 -> OpenRouter); sem aprovacao juridica; Privacy Manifest e respostas do Console nao alterados (decisao humana).
- **Revisao independente (Codex CLI GPT-5.6-Sol, familia OpenAI, conteudo inline):** R1 (b3ea656) P0=1/P1=2/P2=4/P3=1 -> F2 excecao de QA removida, F3 variavel vazia mascarando .env no modo none (normalizacao + teste de integracao em processo filho), F4 esquema same-origin sem x-forwarded-proto, F5 provedor so HTTPS, F6 teto em bytes + Content-Length, F7 excecao do explain_target_change documentada, F8 testes; F1 (gateway publico sem auth/quota) disputado com fatos. R2 (8484d96) P0=0/P1=0/P2=3/P3=1: F1 DISPUTED-ACCEPTED (pre-existente, nao atribuivel ao GOAL; P1 de backlog em PENDENCIAS), F2-F8 FIXED; N1 leitura do corpo com teto por stream, N2 teto da resposta do provedor em bytes, N3 redirect error no fetch do provedor, N4 docs -> corrigidos em b8cfc81 e nesta documentacao. R3 (6baca02) P0=0/P1=0/P2=1/P3=0: N1-N4 FIXED, F2-F8 seguem FIXED; M1 (.env com NEXT_PUBLIC_GYMFLOW_AI_BACKEND_URL= vazio aceito no modo none; o Next embutiria "" e a auditoria reprovaria depois) corrigido no mesmo GOAL (recusa antes do build + teste de integracao que falha sem a correcao). Resultado final da revisao: P0=0, P1=0 atribuiveis ao GOAL; P1 pre-existente (gateway publico) no backlog.
- **Validacoes finais (b8cfc81):** focados NUT-007/GOAL-118 + ferramental verdes; npm test 161 arquivos / 3642 testes PASS (apos M1) (duas execucoes com o emulador ligado falharam por falta de memoria do host: 1 timeout e depois 86 falhas de hidratacao/timeout em testes de storage/Provider; com o emulador desligado, 161/3640 verdes; o teste do timeout passou isolado); npx tsc --noEmit 0 erros; npm run build OK; npm run build:mobile OK (Production embutida, out/ sem segredos); npm run ios:validate 17/17; git diff --check limpo; testes de mutacao confirmam que os testes de F3 e N2 falham sem a correcao.
- **Status:** CORS_ANDROID=PASS; CORS_IOS=PASS; CORS_UNAUTHORIZED_ORIGIN=BLOCKED; BACKEND_PRODUCTION=PASS; BACKEND_PRODUCTION_ORIGIN_EMBEDDED=PASS; AI_NATIVE_RUNTIME=PASS (emulador; S22 nao conectado); AI_NATIVE_PROVIDER_REAL=YES; AI_NATIVE_CURL_SUBSTITUTE=NO; MODEL_MACROS_TRUSTED=NO; AI_CAN_WRITE_TARGETS=NO; USER_CONFIRMATION_REQUIRED=YES; OFFLINE_DEGRADATION=PASS; ONLINE_RECOVERY=PASS (ressalva Wi-Fi virtual, P2 para o S22); DIRECT_OPENROUTER_CLIENT_CALL=NO; SECRET_EXPOSURE=NO; VERSION_CODE=1; VERSION_NAME=1.0; D-NUT-08/09=PENDING; READY_TO_RESUME_GOAL117=YES. Nenhuma acao no Google Play; sem upload key; android:play:release nao executado.
