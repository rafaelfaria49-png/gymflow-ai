...
- **Depois:** gateway com CORS cirurgico so para https://localhost e capacitor://localhost (origem ecoada, nunca *; preflight so POST + Content-Type; CORS em 200/400/403/409/413/502/503/504; Vary), web same-origin e chamadas sem Origin inalteradas, qualquer outra origem -> 403 antes de ler o corpo (provedor nunca chamado); build:mobile com --ai-backend production|none (padrao production, constante versionada, .env so pode repetir a Production); android:play:release passa o modo aprovado ao build:mobile. Deploy Production dpl_9b1V1yrYdvSEUYAn5xoPXjReq2rb (READY, alias https://gymflow-beige-gamma.vercel.app, a partir de worktree limpo do commit 3ef25b6). Runbook/evidencias: docs/mobile/ANDROID_AI_BACKEND_BRIDGE_118.md.
...
- **Android (chave interna, sem upload key):** build:mobile -> cap sync -> gradlew clean assembleRelease bundleRelease -> android:release:audit --no-record --expect-backend-production = 36 PASS / 0 FAIL / 0 WARN (BACKEND_PRODUCTION_ORIGIN_EMBEDDED, BACKEND_MODE_MATCHES_APPROVAL, scans de segredo/dev em APK/AAB/out/). APK f91bdd74...; AAB 3804bd06...; versionCode 1 / versionName 1.0.
