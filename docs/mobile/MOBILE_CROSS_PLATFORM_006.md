...
- **`NSPrivacyCollectedDataTypes`:** `[]`. Zero dados transmitidos para servidores remotos; persistência 100% local em IndexedDB/LocalStorage.
  - **Atualização GOAL-118 (2026-09-25) — premissa superada.** O bundle nativo (o mesmo do Android) passou a enviar, quando o usuário usa o Assistente IA da Nutrição, contexto nutricional mínimo ao gateway GymFlow Production (`capacitor://localhost` → `https://gymflow-beige-gamma.vercel.app/api/nutrition/assistant` → OpenRouter). Campos e ausências: `docs/mobile/MOBILE_CROSS_PLATFORM_005.md` §9. Pela definição da Apple, só é "coleta" o que fica acessível além do necessário para atender a requisição em tempo real: o gateway não persiste nada, mas a retenção no OpenRouter/provedor do modelo **não foi auditada**. O manifesto **não** foi alterado neste GOAL; declarar ou não `NSPrivacyCollectedDataTypes` (ex.: Health/Fitness, "App Functionality", não vinculado à identidade, sem tracking) e as App Privacy Details do App Store Connect é **decisão humana/jurídica** (D-NUT-09 `PENDING`) antes de qualquer submissão iOS. O ATS continua satisfeito (HTTPS).
